...
def create_output_csv_file_path():
    ...
    temp_output_dir = tempfile.mkdtemp()  # Creates a temporary directory
    temp_output_file_path = os.path.join(temp_output_dir, 'output.csv')
    # The directory remains after the function call; remember to clean it up when no longer needed
...
def cms_instance():
    ...
    yield cms
    for path in [DB_PATH, PATH_INPUT_CSV_FILE]:
...
def test_output_file_creation_and_contents(cms_instance):
    ...
    print(PATH_INPUT_CSV_FILE)
    print(PATH_OUTPUT_CSV_FILE)
